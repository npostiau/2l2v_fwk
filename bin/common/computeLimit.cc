  ...
  void AllInfo_t::addHardCodedUncertainties(string histoName){
    ...
    for(unsigned int p=0;p<sorted_procs.size();p++){
      ...
      for(std::map<string, ChannelInfo_t>::iterator ch = it->second.channels.begin(); ch!=it->second.channels.end(); ch++){
        ...
        if(ch->second.shapes.find(histoName)==(ch->second.shapes).end())continue;
        ShapeData_t& shapeInfo = ch->second.shapes[histoName];      
        if(it->second.shortName.find("ggH_sonl")!=string::npos && chbin.Contains("eq0jet" )){integral_sonl[0] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_sonl")!=string::npos && chbin.Contains("eq1jet" )){integral_sonl[1] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_sonl")!=string::npos && chbin.Contains("vbf" )){integral_sonl[2] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_bonl")!=string::npos && chbin.Contains("eq0jet" )){integral_bonl[0] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_bonl")!=string::npos && chbin.Contains("eq1jet" )){integral_bonl[1] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_bonl")!=string::npos && chbin.Contains("vbf" )){integral_bonl[2] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_sand")!=string::npos && chbin.Contains("eq0jet" )){integral_sand[0] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_sand")!=string::npos && chbin.Contains("eq1jet" )){integral_sand[1] = shapeInfo.histo()->Integral();}
        if(it->second.shortName.find("ggH_sand")!=string::npos && chbin.Contains("vbf" )){integral_sand[2] = shapeInfo.histo()->Integral();}
      }
    }
    for(unsigned int p=0;p<sorted_procs.size();p++){
      string procName = sorted_procs[p];
